...
class SimplyEco {
	...
	async RemoveItem(GuildID, ItemID) {
		if (!GuildID) throw new TypeError('A GuildID must be specified');
		if (!ItemID) throw new TypeError('A Item Id must be specified.');
		let GuildData = await Guildd.findOne({
			gid: GuildID
		});
		if (!GuildData) {
	...
	async BuyItem(UserID, GuildID, ItemId) {
		...
		let item = await Shop.shopItems.filter(item => item.id === ItemId);
		let itemName = await Shop.shopItems.find(item => item.id === ItemId);
		...
		if (!UserData) {
			...
			let item1 = UserData.inventory.find(item => item.Name === ItemId);
	...
	async getItem(guildId, itemName) {
		if (!guildId) throw new TypeError('A guild id must be specified.');
		if (!itemName) throw new TypeError('No item name or id specified.');
		let GuildData = await Guildd.findOne({
			gid: guildId
		});
		if (!GuildData) return null;
		return GuildData.shopItems.filter(item => item.id == itemName || item.Name == itemName)[0];
	}
	/**
	 * GetBankbal
	 * @param {String} UserID
