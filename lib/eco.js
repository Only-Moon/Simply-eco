...
class SimplyEco {
	...
	async RemoveItem(GuildID, Item) {
		...
		GuildData.shopItems = GuildData.shopItems.filter(item => item.id !== Item || item.Name !== Item);
	...
	async SellItem(UserID, GuildID, ItemN) {
		...
		let itemName = await Shop.shopItems.filter(item => _checkItem(item, ItemN));
		let UserData = await data.findOne({
			gid: GuildID,
			userID: UserID
		});
	...
	async getItem(guildId, itemName) {
		...
		return GuildData.shopItems.filter(item => _checkItem(item, itemName))[0];
	}
	/**
	 * GetBankbal
	 * @param {String} UserID
