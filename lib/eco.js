...
const Guildd = require('../models/nonGlobal/guild');
const gshop = require('../models/global/global-shop');
const guserData = require('../models/global/global-users');
	...
	*/
	...
	async RemoveJob({ GuildID, userId }) {
		...
		if (!this.options?.global) {
			...
			if (!user) {
				const aa = new guildUserData({ userID: userId, gid: GuildID });
				return aa.save();
			}
